...
const Super = {
  ...
  chatbot: {
    ...
    KB: [
      ...
      { m: ['sample data', 'demo data', 'fill demo', 'empty pages', 'no records yet'], r: 'Admins: open **Admin Data → 🎬 One-Click Demo Sample Data** and press the button — payroll, inventory, application links, messages, assignments, behaviour points, support plans, library books, help-desk tickets, house points, cafeteria menus, lost & found and PTA meetings are all filled with believable rows in seconds. Idempotent: tables that already have data are skipped. Guaranteed alternative: paste database/demo-sample-data.sql in the Supabase SQL Editor once. On demo deployments the pack also loads itself automatically each day.', p: 'admin-data.html' },
      ...
      { m: ['deleted student still', 'ghost report', 'old report sheet', 'wrong column report', 'stale scores'], r: 'Two clean-up tools: ① deleting a student now auto-archives their full record (Activity/Admin Data → student_archive) and removes their loose score rows, freeing the admission number for reuse; ② on **Report Cards**, admins click **🧹 Sweep Ghost Report Sheets** to purge leftovers from deleted/re-created columns or deleted students — then regenerate and only current data prints.', p: 'report-cards.html' },
      { m: ['same name students', 'admission number report', 'duplicate name'], r: 'Report generation is admission-number aware: the student box on **Report Cards** now offers "ADM/NO — Name (Class)" suggestions; type either the name or the admission number. Two students sharing a name can never be confused, and class/term/session pickers let you generate PREVIOUS terms too.', p: 'report-cards.html' },
      { m: ['seo', 'google search', 'searchable', 'bing', 'index my site', 'search engine'], r: 'Every site ships crawl-ready (robots.txt, sitemap, meta tags, School structured-data with HMG ecosystem links). To appear on Google/Bing: follow **docs/SEO-AND-LEAD-GENERATION-GUIDE.md** — verify the domain in Google Search Console (HTML tag method), submit sitemap.xml, request indexing, then import into Bing Webmaster Tools. ~20 minutes per school.' },
      { m: ['fees permission', 'who records fees', 'bursar fees'], r: 'Fees & Payments: ordinary staff/teachers are READ-ONLY. The ADMIN TIER — bursar, principal, head teacher, proprietor, admin — records payments, edits and prints receipts. Students see only their own payments; parents only their children\'s.', p: 'fees.html' },
      { m: ['complaint status', 'who resolves complaint', 'grievance workflow'], r: 'Complaints: parents/students submit the complaint (category, details, urgency, evidence link) — the Status, Assigned-to and Resolution fields are hidden from them and belong to the ADMIN, who reviews, assigns and closes with a resolution note. Complainants watch the status change.', p: 'complaints.html' },
      { m: ['current term card', 'profile report card', 'auto report card'], r: 'From a student\'s profile, clicking **📄 Report card** now auto-selects the CURRENT term & session (from Academic Setup\'s current period) and generates that student\'s card immediately — zero manual picking. **🪪 ID card** preselects the student the same way.', p: 'student-profile.html' },
      { m: ['deleted student report', 'ghost still prints', 'roster gate'], r: 'Three independent shields now guarantee deleted students never print: ① deletion archives the record and purges leftovers (admission number freed), ② admins can 🧹 Sweep Ghost Report Sheets for old databases, ③ the report engine itself has a LIVE ROSTER GATE — any score row whose student is no longer in the register is dropped at render time, on every database, always.', p: 'report-cards.html' },
      { m: ['promotion pending', 'still pending report', 'promoted not showing', 'apply promotion error'], r: 'Fixed in V7.3: report cards now recognise **applied** promotions (the state “✅ Apply promotions” creates) and fall back gracefully when a decision was saved without a term. The Apply button also finalises server-side (`sc_finalise_promotions`): moves each student to the new class by ID, stamps the CURRENT term/session on the decision, marks graduates — so the card immediately prints PROMOTED TO…, REPEAT or GRADUATED. If your database is older, run the latest complete-schema.sql once.', p: 'promotion.html' },
      { m: ['auto fill promotion', 'to class automatic', 'next class automatic', 'promotion form'], r: 'The Promotion form now completes itself: pick the student and the platform auto-fills From-class, **To-class (the next rung on YOUR school\'s own class ladder — Primary → JSS → SS)**, the action (promote, or GRADUATE automatically for the final class), status approved, and the current term/session. Auto-promote\'s modal also pre-selects the current period and suggests the graduating class. Everything stays editable — you review, then ✅ Apply.', p: 'promotion.html' },
      { m: ['auto term', 'auto session', 'stop typing term', 'current period forms'], r: 'Every NEW record form now auto-selects the school\'s CURRENT term and session (set once in Academic Setup → Academic Period → “Set as current”). You never type the period again; edits keep their stored values and every auto-choice remains changeable.' },
    ],
